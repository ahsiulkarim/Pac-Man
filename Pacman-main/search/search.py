...
def depthFirstSearch(problem):
    ...
    # Push: (state, path_taken)
    stack.push((problem.getStartState(), []))

    while not stack.isEmpty():
        state, path = stack.pop()

        # If goal found, return actions
        if problem.isGoalState(state):
            return path

        # If not visited, mark visited and expand
...
def uniformCostSearch(problem):
   ...
   # util.raiseNotDefined()
    ...
    # PriorityQueue items: (state, path, costSoFar)
    pq = PriorityQueue()
    ...
    while not pq.isEmpty():
        state, path, costSoFar = pq.pop()

        # If we reached goal, return result
        if problem.isGoalState(state):
            return path

        # If we have seen state with a lower cost, skip
        if state in visited_costs and visited_costs[state] <= costSoFar:
            continue

        visited_costs[state] = costSoFar

        for nextState, action, stepCost in problem.getSuccessors(state):
            newCost = costSoFar + stepCost
            # Only push if this path is better than any known path
            if nextState not in visited_costs or newCost < visited_costs[nextState]:
                pq.push((nextState, path + [action], newCost), newCost)
